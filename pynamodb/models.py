...
from pynamodb.constants import (
    ...
    COUNT, ITEM_COUNT, UNPROCESSED_ITEMS
)
...
class BatchWrite(ModelContextManager):
    ...
    def commit(self):
        ...
        while unprocessed_items:
            timeout = timeout * 2
            if timeout < 16:
                log.debug("BATCH WRITE PARTIALLY THROTTLED: At capacity, exponentially backing off for %s seconds", timeout)
            else:
    ...
    def batch_get(cls, items, consistent_read=None, attributes_to_get=None):
        ...
        while keys_to_get:
            ...
            if unprocessed_keys:
                ...
                timeout = timeout * 2
                if timeout < 16:
                    log.debug("BATCH READ PARTIALLY THROTTLED: At capacity, exponentially backing off for %s seconds", timeout)
                else:
